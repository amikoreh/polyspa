...
function polyspa_customize( $wp_customize ) {
 ...
 */
	...
	$wp_customize->add_control( new WP_Customize_Image_Control($wp_customize, 'header_logo', array(
		'label'       => __( 'Upload Header Logo', 'polyspa' ),
		'description' => __( 'Height: &gt;80px', 'polyspa' ),
	...
	$wp_customize->add_control( 'search_enabled', array(
		'type'          => 'checkbox',
		'label'         => __( 'Show Searchfield?', 'polyspa' ),
		'section'       => 'theme_header_section',
		'settings'   => 'search_enabled',
		'priority'   => 2,
	) );
}
add_action( 'customize_register', 'polyspa_customize' );


/**
 * Bind JS handlers to make Theme Customizer preview reload changes asynchronously.
 */
function polyspa_customize_preview_js() {
	wp_enqueue_script( 'customizer', get_template_directory_uri() . '/inc/customizer.js', array( 'jquery' ), null, true );
}
add_action( 'customize_preview_init', 'polyspa_customize_preview_js' );
