...
<dom-module id="post-list" attributes="show">
	<template>
		...
		<paper-radio-group id="change_order" selected="date|desc" on-paper-radio-group-changed="_orderChanged">
			<paper-radio-button name="date|desc"><?php _e( 'Order by Date', 'polyspa' ); ?></paper-radio-button>
			<paper-radio-button name="title|asc"><?php _e( 'Order by Title', 'polyspa' ); ?></paper-radio-button>
		</paper-radio-group>
		...
		<section class="flex layout vertical">
			<template is="dom-repeat" items="{{data}}">
				...
				<div id="{{item.id}}">
					...
					<footer>
						<small><?php _e( 'Posted on', 'polyspa' ); ?> <em>{{formatTimestamp(item.date)}}</em></small>
					</footer>
				</div>
			</template>
		</section>
